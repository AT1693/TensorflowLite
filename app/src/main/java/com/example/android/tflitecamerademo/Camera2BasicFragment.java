...
public class Camera2BasicFragment extends Fragment
  ...
  public void onStart() {
    super.onStart();
    textToSpeechSystem=new TextToSpeech(getActivity(), new TextToSpeech.OnInitListener() {
      @Override
      public void onInit(int status) {
        if(status != TextToSpeech.ERROR) {
          textToSpeechSystem.setLanguage(Locale.UK);
          textToSpeechSystem.setSpeechRate(3.0f);
...
//                        CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_ON_AUTO_FLASH);
  ...
  private void classifyFrame() {
    ...
    if (classifier == null || getActivity() == null || cameraDevice == null) {
      showToast("Starting \ud83d\udeeb \nWait for some time \ud83d\ude4b\u200d\u2642\ufe0f");
      return;
    }
    Bitmap bitmap =
        textureView.getBitmap(ImageClassifier.DIM_IMG_SIZE_X, ImageClassifier.DIM_IMG_SIZE_Y);
    String textToShow = classifier.classifyFrame(bitmap);
    String []textToShowa= textToShow.split("@@");
    final GlobalClass globalVariable = (GlobalClass) getActivity().getApplicationContext();

    //Set name and email in global/application context
    globalVariable.setName(textToShowa[1]);

      if (this.looper%101==0) {
        this.looper=0;
      }

      else {
        if (this.looper%100==0) {
//            try{
//            this.manager.setTorchMode(cameraId, true);}
//            catch (Exception e){
//                Log.d("1","Er1"+e);
//            }
          ...
          if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            //textToSpeechSystem.speak(textToShowa[1], TextToSpeech.QUEUE_FLUSH, null, null);
          } else {
            //textToSpeechSystem.speak(textToShowa[1], TextToSpeech.QUEUE_FLUSH, null);
    ...
    String textToShows=textToShowa[0];

    bitmap.recycle();
    showToast(textToShows);
  }
